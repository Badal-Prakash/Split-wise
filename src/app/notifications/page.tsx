  ...
  return (
    <AppShell>
      <section className="space-y-5 p-4 md:p-8">
        ...
        {notifications.isLoading ? <LoadingRows /> : notifications.data?.length ? <div className="space-y-3">{notifications.data.map((notification) => <div key={notification._id} className={`card flex items-start justify-between gap-3 ${notification.readAt ? "opacity-70" : ""}`}><div className="flex gap-3"><div className="rounded-xl bg-emerald-50 p-3 text-emerald-600 dark:bg-emerald-950"><Bell size={18} /></div><div><p className="font-semibold">{notification.title}</p><p className="text-sm text-slate-500">{notification.body}</p><p className="mt-1 text-xs text-slate-400">{new Date(notification.createdAt).toLocaleString("en-IN",{timeZone:"Asia/Kolkata"})}</p>{notification.type === "group.invite" && notification.payload?.inviteId && <div className="mt-3 flex flex-wrap gap-2"><button onClick={() => updateInvite.mutate({ id: notification.payload.inviteId, status: "accepted" })} className={buttonClass}>Accept</button><button onClick={() => updateInvite.mutate({ id: notification.payload.inviteId, status: "rejected" })} className={ghostButtonClass}>Reject</button></div>}</div></div><div className="flex gap-1"><button onClick={() => markRead.mutate({ id: notification._id, read: !notification.readAt })} className={ghostButtonClass}>{notification.readAt ? "Unread" : "Read"}</button><button onClick={() => remove.mutate(notification._id)} className="rounded-xl p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950"><Trash2 size={16} /></button></div></div>)}</div> : <EmptyState title="No notifications" body="Expense alerts, settlement reminders, invites, and comment mentions will appear here." />}
      </section>
    </AppShell>
  );
}
