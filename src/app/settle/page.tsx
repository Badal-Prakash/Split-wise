...
export default function SettlePage() {
  ...
  const edgeLabel = (item: any) => `${item.fromUserName ?? item.fromUser} pays ${item.toUserName ?? item.toUser} ${formatMoney(item.amount, me.data?.currency ?? "INR")}`;
  const userName = (user: any) => user?.name ?? user?.email ?? user ?? "Member";
  const settle = useMutation({ mutationFn: () => apiRequest("/api/settlements", { method: "POST", body: JSON.stringify({ fromUser: edge.fromUser, toUser: edge.toUser, amount: Number(amount || edge.amount), currency: me.data?.currency ?? "INR", paymentMethod, status: Number(amount || edge.amount) < edge.amount ? "partial" : "settled", notes }) }), onSuccess: () => { setAmount(""); setNotes(""); queryClient.invalidateQueries(); } });
