  ...
  return (
    <AppShell>
      <section className="space-y-5 p-4 md:p-8">
        ...
        {tab === "analytics" && <div className="space-y-4"><div className="card"><p className="text-sm text-slate-500">Total group spending</p><b className="text-2xl">{formatMoney(analytics.data?.categories?.reduce((sum: number, row: any) => sum + row.total, 0) ?? 0, currency)}</b></div><div className="grid gap-4 lg:grid-cols-2"><div className="card"><h3 className="mb-3 font-semibold">Monthly spending</h3>{analytics.data?.monthly?.length ? <MonthlyChart data={analytics.data.monthly} /> : <EmptyState title="No analytics yet" />}</div><div className="card"><h3 className="mb-3 font-semibold">Category mix</h3>{analytics.data?.categories?.length ? <CategoryChart data={analytics.data.categories} /> : <EmptyState title="No category data" />}</div><div className="card lg:col-span-2"><h3 className="mb-3 font-semibold">Member contributions</h3><div className="space-y-2">{(analytics.data?.userContribution ?? []).map((row: any) => <div key={row.userId} className="flex items-center justify-between rounded-xl bg-slate-50 p-3 text-sm dark:bg-slate-900"><span>{users.find((u: any) => String(u?._id ?? u?.id) === row.userId)?.name ?? row.userId}</span><b>{formatMoney(row.total, currency)}</b></div>)}{analytics.data?.userContribution?.length === 0 && <EmptyState title="No contribution data" />}</div></div></div></div>}
