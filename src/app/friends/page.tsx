  ...
  return (
    <AppShell>
      <section className="space-y-5 p-4 md:p-8">
        ...
        {friends.isLoading ? <LoadingRows /> : friends.data?.friendships?.length ? <div className="grid gap-4 lg:grid-cols-2">{friends.data.friendships.map((friendship: any) => <div key={friendship._id} className="card space-y-3"><div className="flex items-start justify-between"><div><p className="font-semibold">Friendship</p><p className="text-sm capitalize text-slate-500">{friendship.status}</p></div><button onClick={() => remove.mutate(friendship._id)} className="rounded-xl p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950"><X size={16} /></button></div><div className="flex gap-2">{friendship.status === "pending" && <button onClick={() => update.mutate({ id: friendship._id, status: "accepted" })} className={buttonClass}>Accept</button>}<button onClick={() => update.mutate({ id: friendship._id, status: "blocked" })} className={ghostButtonClass}>Block</button></div></div>)}</div> : <EmptyState title="No friends yet" body="Search for users and add them to create direct expenses and settle balances." />}
        <div className="card"><h3 className="mb-3 font-semibold">Friend balances</h3>{friends.data?.balances?.length ? friends.data.balances.map((edge: any, index: number) => <div key={index} className="mb-2 flex justify-between rounded-xl bg-slate-50 p-3 text-sm dark:bg-slate-900"><span>{edge.fromUser} pays {edge.toUser}</span><b>{formatMoney(edge.amount)}</b></div>) : <StatusBanner>No direct friend balances yet.</StatusBanner>}</div>
      </section>
    </AppShell>
  );
}
