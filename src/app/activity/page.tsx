  ...
  return (
    <AppShell>
      <section className="space-y-5 p-4 md:p-8">
        ...
        <div className="card grid gap-3 md:grid-cols-2"><Field label="Group"><select value={groupId} onChange={(event) => setGroupId(event.target.value)} className={inputClass}><option value="">All groups</option>{groups.data?.map((group) => <option key={group._id} value={group._id}>{group.name}</option>)}</select></Field><Field label="Type"><select value={type} onChange={(event) => setType(event.target.value)} className={inputClass}>{types.map((item) => <option key={item || "all"} value={item}>{item || "All activity"}</option>)}</select></Field></div>
        <div className="card">{activities.isLoading ? <LoadingRows /> : activities.data?.length ? <div className="space-y-2">{activities.data.map((activity) => <div key={activity._id} className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900"><p className="font-medium">{activity.type.replaceAll(".", " ")}</p><p className="text-sm text-slate-500">{new Date(activity.createdAt).toLocaleString()}</p><pre className="mt-2 overflow-auto text-xs text-slate-500">{JSON.stringify(activity.payload ?? {}, null, 2)}</pre></div>)}{activities.data.length >= limit && <button onClick={() => setLimit((value) => value + 30)} className="mt-3 w-full rounded-xl border border-slate-200 p-3 text-sm font-semibold dark:border-slate-800">Load more</button>}</div> : <EmptyState title="No matching activity" />}</div>
      </section>
    </AppShell>
  );
}
